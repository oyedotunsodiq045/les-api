...
// @desc    Export all Users (SME's and Non SME's) as CSV
...
exports.exportUsers = asyncHandler(async (req, res, next) => {
  ...
  const users = await User
    ...
    .find({ role: { $in: ['user', 'sme'] } })
    // sort them by their name,
    .sort({ name: 1 })
    // pick only their name and etc,
    .select({ name: 1, email: 1, phone: 1, businessName: 1, website: 1, address: 1, role: 1 });
...
// @desc    Export all Users (Non SME's) as CSV
